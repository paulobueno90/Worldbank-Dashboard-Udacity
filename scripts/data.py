...
def return_figures(countries=country_default,
                   ...
                   initial_date='1990',
                   final_date=str(dt.datetime.today().year)):
    ...
    for indicator in indicators:
        url = 'http://api.worldbank.org/v2/countries/' + country_filter + \
              '/indicators/' + indicator + f'?date={initial_date}:{final_date}&per_page=1000&format=json'
        urls.append(url)
    ...
    layout_one = dict(title=f'Change in Hectares Arable Land <br> per Person {initial_date} to {final_date}',
    ...
    graph_two = []
    df_one.sort_values('value', ascending=False, inplace=True)
    df_one = df_one[df_one['date'] == final_date]
    ...
    layout_two = dict(title=f'Hectares Arable Land per Person in {final_date}',
    ...
    df_three = df_three[(df_three['date'] == final_date) | (df_three['date'] == '1990')]
    ...
    layout_four = dict(title=f'% of Population that is Rural versus <br> % of Land that is Forested <br> {initial_date}-{final_date}',
